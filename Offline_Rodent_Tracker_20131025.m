function Offline_Rodent_Tracker
...
handles.thresh = [];                                                        %Create a field to hold the detection threshold
handles.buffsize = 5;                                                       %Set the reference buffer to hold 5 seconds of reference frames.
handles.filter_size = 1;                                                    %Set the default filter size, in centimeters.
...
text(min(get(handles.axes,'xlim')),min(get(handles.axes,'ylim')),...
    ...
    'fontweight','bold','color','c');                                       %Create top-left text object explaining how to calibrate the scale.
...
if isempty(val)                                                             %If the user didn't enter a length...
    delete(handles.scale_line{handles.cur_vid});                            %Delete the scale markers.
    return;                                                                 %Skip execution of the rest of the function.
end
...
set(handles.trackbutton,'string','Running Analysis...');                    %Change the string on the start analysis button.
set([handles.loadbutton,handles.listbox,handles.slider,...
    handles.trackbutton],'enable','off');                                   %Disable the other uicontrols.
for c = 1:size(handles.vidfiles,1)                                          %Step through each video.
    ...
    filter_size = round(handles.filter_size*scale);                         %Calculate the filter size to use, in pixels.
    ...
    temp = ref_im(bounds(2):bounds(4),bounds(1):bounds(3));                 %Grab only the image within the boundary.
    ref_im = ref_im - min(temp(:));                                         %Subtract out the minimum value from the reference image.
    ref_im = ref_im/max(temp(:));                                           %Normalize the reference image.
    set(handles.im,'cdata',ref_im);                                         %Update the 'CData' property of the image to the new frame.
    ...
    temp = [handles.vidfiles{c,1}, rat, '_ACTIVITY.txt'];                   %Create the text file name.
    fprintf(1,'Save path data to: %s\n',temp);                              %Show the user the data file destination.
    fid = fopen(temp,'wt');                                                 %Open a new text file to recieve tracking information.
    ...
    while curframe < handles.vid.NumberOfFrames                             %Loop until we've gone through all the frames.
        ...
        if plot_type == 0                                                   %If the plot type is zero...
            set(handles.im,'cdata',im);                                     %Update the 'CData' property of the image to show the normalized image.
        end
        im = medfilt2(im,[1,1]*filter_size);                                %Filter the image to remove noise pixels.
        temp = im(bounds(2):bounds(4),bounds(1):bounds(3));                 %Grab only the image within the boundary.
        ...
        if plot_type == 2                                                   %If the plot type is two...
            set(handles.im,'cdata',im);                                     %Update the 'CData' property of the image to show the difference.
        end
        thresh = handles.thresh*(max(im(:)) - min(im(:))) + min(im(:));     %Calculate a threshold.
        im = (im > thresh);                                                 %Convert the  image to black and white using that grayscale level.
%         im = medfilt2(im,[1,1]*handles.filter_size);                        %Filter the image to remove noise pixels.
        im = bwareaopen(im,handles.min_pixels);                             %Remove small objects less than 10 pixels.
        if plot_type == 3                                                   %If the plot type is three...
            set(handles.im,'cdata',im);                                     %Update the 'CData' property of the image to show the filtered, thresholded image.
        end
        ...
        if ~isempty(objs)                                                   %If any objects are detected...
            ...
            a = (temp(:,1) < bounds(2) | temp(:,3) > bounds(2) |...
...
set([handles.loadbutton,handles.listbox,handles.slider,...
    handles.trackbutton],'enable','on');                                    %Re-enable the other uicontrols.
