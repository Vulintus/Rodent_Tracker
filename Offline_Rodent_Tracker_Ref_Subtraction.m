function Offline_Rodent_Tracker_Ref_Subtraction
...
%         im = mean(im,3);                                                    %Convert the image to grayscale.
    ...
    temp = ref_im(bounds(3):bounds(4),bounds(1):bounds(2));                 %Grab only the image within the boundary.
...
%         im = mean(im,3);                                                    %Convert the image to a 16-bit integer for analysis.
        ...
        if ~isempty(objs)                                                   %If any objects are detected...
            ...
            s = handles.vid.FrameRate*(d/scale)/(curframe - prev_frame);    %Calculate the frame-to-frame speed of the rat.
